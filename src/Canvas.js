...
class Canvas extends Component {
  ...
  constructor(props) {
    const {clickAction, posArr, CVS_WIDTH, CVS_HEIGHT, NUM_CELLS, ...rest} = props;
    ...
    updateDraw(frameCount, timeRate) {
      ...
      */
      ...
      this.setState({var1: ug.var1});

      console.log("ug.var1 after incrementVar() = ", ug.var1);

      //this.setState({count: ug.count});
  ...
  isValidColor(strColor) {
    ...
    let ug = new UniformGrid(posArr, CVS_WIDTH, CVS_HEIGHT, NUM_CELLS);
    // const draw = props.draw;
    // const updateDraw = props.updateDraw;
    
    //console.log("draw=", draw);
    //console.log("updateDraw=", updateDraw);
  const {canvasRef, clickFunction} = useCanvas({key, this.draw, this.updateDraw})
